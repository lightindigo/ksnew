...
class model_ks
{
    public function getAddress($type,$dat)
    {
        ...
        while($r = _mysql::read_row($q))
        {
            ...
            {
                if(!is_numeric($key)) {$res[count($res)-1][$key] = $val;}
            }
        }
    ...
    public function works($parent)
    {
        ...
        $q = "SELECT * FROM `works` WHERE {0} ORDER BY `id`;";
    ...
    public function catalog_contractor_add($args)
    {
        ...
        if(_mysql::count($q) > 0) {return sdk::toApi('404','Name Already Used');}
    ...
    public function catalog_price_list_get($id=false)
    {
        ...
        while($r = _mysql::read_row($q))
        {
            ...
            if($cont['code'] != '200'){ return $cont;}else{$res[count($res)-1]['contactor'] = $cont['data'];}
            $cont = $this->catalog_contractor_get_data($res[count($res)-1]['customer']);
            if($cont['code'] != '200'){ return $cont;}else{$res[count($res)-1]['customer'] = $cont['data'];}
        }
        return sdk::toApi('200','ok',$res);
    }
    public function catalog_price_add($args)
    {
        ...
        if(_mysql::count($q) > 0) {return sdk::toApi('404','Name Already Used');}
        $q = "INSERT INTO `price` (`id`,`name`) VALUES(NULL,'".$args['name']."');";
        if(!($q = _mysql::query($q)))
        {
            return sdk::toApi('10','Invalid query [getCatObject][1]',array(_mysql::getLastError()));
        }
    ...
    public function catalog_price_getdata($id)
    {
        ...
        $cont = $this->catalog_contractor_get_data($res['contactor']);
            if($cont['code'] != '200'){ return $cont;}else{$res['contactor'] = $cont['data'];}
            $cont = $this->catalog_contractor_get_data($res['customer']);
            if($cont['code'] != '200'){ return $cont;}else{$res['customer'] = $cont['data'];}
    ...
    public function save_ks_drs($args)
    {
        ...
        $q = "INSERT INTO `ks` (`cable`,`contract`,`number`,`date_start`,`date_stop`,`name`) VALUES('{0}','{1}','{2}','{3}','{4}','{5}');";
        $qArgs = array($args['project'],$args['price'],$args['doc_n'],$args['date_a'],$args['date_b'],$args['work_name']);
    ...
    public function save_ks_ms($args)
    {
        ...
        foreach(explode("\n",$args['compens']) as $ln)
        {
            ...
            if(!($q = _mysql::query($q,$qArgs)))
            {
                return sdk::toApi('10','Invalid query [save_ks_drs][7]',array(_mysql::getLastError()));
            }
        }
        return sdk::toApi('200','ok',array('ks'=>$id));
    }
    ...
    public function ks_data($id)
    {
        ...
        if($mkPrice['code'] != '200') {return $mkPrice;}
        ...
        if(_mysql::count($q) > 0)
        {
            ...
            {
                $res_tmp[] = array();
                foreach($r as $key => $val){if(!is_numeric($key)) {$res_tmp[count($res_tmp)-1][$key] = $val;}}
    ...
    private function display_ks11($res)
    {
        ...
        $args = array();
        $args['date_start'] = $res['date_start'];
        $args['date_stop'] = $res['date_stop'];
        ...
        $args['contract_date'] = $res['contract']['contact_date'];
        $args['accord'] = $res['contract']['accord_number'];
        $args['accord_date'] = $res['contract']['accord_date'];
        //content
        $args['total_nds'] = 0;
        $args['nds'] = 0;
        $args['total'] = 0;
    ...
    public function price_item($act,$args)
    {
        ...
        {
            $res[] = array();
            foreach($r as $key => $value){if(!is_numeric($key)) {$res[count($res)-1][$key] = $value;}}
            ...
            {
                foreach($r2 as $key => $value){if(!is_numeric($key)) {$res2[$key] = $value;}}
            }
